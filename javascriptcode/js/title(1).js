...
function title_show(pSpan) {
    ...
    divObj.innerHTML = "<span id='title_show' style='position:absolute;display:none;background-color:#f0f0f0;z-index:999;opacity:1;font-size:12px;color:#999999;padding:10px;border:1px solid #f5f5f5;border-radius:3px;line-height:18px'></span>";
    ...
    if(pageX(span)+260>document.body.offsetWidth){
        div.style.right = document.body.offsetWidth - pageX(span) - 10 + 'px';
    }else{
        div.style.left = pageX(span) + 'px';
    }
    div.style.top = pageY(span) + 20 + 'px';
    var words_per_line = 1000;     //每行字数   
