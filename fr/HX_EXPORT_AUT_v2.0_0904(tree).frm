...
<Query>
<![CDATA[
select  distinct u.username user_id, u.realname||'('||u.username||')' user_name
...
--查询角色与报表权限关系
asd as 
 (select C.id, c.rolename ROLENAME, f.name aname, e.name bname, e.id bid
    from FR_T_CUSTOMROLE_V c --角色信息
  
   inner join FR_T_CUREP_V d --角色权限
      on c.id = d.roleid
     and view1 = 1
  
    left join FR_REPORTLETENTRY_V e --报表
      on d.entryid = e.parent
  
    left join FR_FOLDERENTRY_V f
      on e.parent = f.id
  union all
  select C.id, c.rolename ROLENAME, f.name aname, e.name bname, e.id bid
    from FR_T_CUSTOMROLE_V c --角色信息
  ...
  --用户关联角色，查询用户的报表权限
c as
 (SELECT DISTINCT TYPE1, bname
    FROM (select distinct case
                            when a.aname = '经营管控' then
                             'jy'
                            when a.aname = '资产管理' then
                             'zy'
                            when a.aname = '住宅端到端' then
                             'zz'
                          --when a.aname='产业端到端' then 'cy' 
                          end type1,
                          a.bname
          
            from Fr_t_Customrole_User_v f
            left join FR_T_USER_V u
              on u.id = f.Userid
            left join FR_T_CUSTOMROLE_V c
              on c.id = f.CustomRoleid
            left join asd a
              on f.CustomRoleid = a.id
           where a.bname is not null
             --AND u.username = '${p_user}'
             ${if(len(p_user)=0, " and 1=1 ", " and u.username='"+p_user+"' " )}
             and a.aname in ('经营管控', '住宅端到端', '资产管理')
          union all
          select distinct case
                            when C.NAME = '经营管控' then
                             'jy'
                            when C.NAME = '资产管理' then
                             'zy'
                            when C.NAME = '住宅端到端' then
                             'zz'
                          --when a.aname='产业端到端' then 'cy' 
                          end type1,
                          b.name
           ...
           ${if(len(p_user)=0, " and 1=1 ", " and u.username='"+p_user+"' " )}
             ...
             AND A.entryid = B.id
             AND B.parent = C.id
             and C.NAME in ('经营管控', '住宅端到端', '资产管理'))),
--查询父级
b as
 (select distinct ee.level2code, c.*
    from c, DM_EXPORT_PRIVILEGE ee
   where c.bname = ee.level2module
     and ee.classify = 'tab'),
--查询树
d as
 (select *
    from (select distinct a.level1module levelmodule,
          ...
          select distinct a.tab_code levelmodule,
            ...
            from DM_EXPORT_PRIVILEGE a
           where classify = 'tab'
           and a.codes<>'jycy004'  --签约落地投
           )
  
   where 1 = 1)

select *
  from (select d.levelmodule, d.levelcode, d.father_id
          from d, b
         where 1 = 1
           and father_id = b.level2code
        union all
        select d.levelmodule, d.levelcode, d.father_id
          from d, b
         where 1 = 1
           and levelcode = level2code
        union all
        select distinct d.levelmodule, d.levelcode, d.father_id
          from d, b
         where 1 = 1
           and levelcode = b.type1)
 order by levelcode
]]></Query>
</TableData>
...
select *
  from (select distinct levelmodule, levelcode, codes, father_id
          from d, b
         where 1 = 1
           and d.levelcode = b.type1
        union all
        select distinct levelmodule, levelcode, codes, father_id
          from d, b
         where 1 = 1
           and d.levelcode like b.level2code || '%')
 order by levelcode
]]></Query>
</TableData>
...
<Attributes>
<![CDATA[=if(len($p_export)=0, "no", $p_export)]]></Attributes>
</O>
</ColumnConfig>
...
//var sql22=" with asd as (select C.id, c.rolename ROLENAME, f.name  aname, e.name  bname, e.id  bid from FR_T_CUSTOMROLE_V c  inner join FR_T_CUREP_V d  on c.id = d.roleid and view1 = 1 left join FR_REPORTLETENTRY_V e  on d.entryid = e.parent left join FR_FOLDERENTRY_V f on e.parent = f.id union all select C.id, c.rolename ROLENAME, f.name   aname, e.name bname, e.id  bid from FR_T_CUSTOMROLE_V c  inner join FR_T_CUREP_V d  on c.id = d.roleid and view1 <> 0 left join FR_REPORTLETENTRY_V e  on d.entryid = e.id left join FR_FOLDERENTRY_V f on e.parent = f.id ), b as (select distinct case when a.aname = '经营管控' then 'jy' when a.aname = '资产管理' then 'zy' when a.aname = '住宅端到端' then 'zz' when a.aname = '产业端到端' then 'cy' end type1 from FR_T_CustomRole_User_V f left join FR_T_USER_V u on u.id = f.Userid left join FR_T_CUSTOMROLE_V c on c.id = f.CustomRoleid left join asd a on f.CustomRoleid = a.id where a.bname is not null AND u.username = '"+v_user+"' and a.aname in ('经营管控', '产业端到端', '住宅端到端', '资产管理')) select ' [ ''' || substr(levelcode, 1, 2) || '''' || ',' || '''' || father_id || '''' || ',' || '''' || levelcode || '''' || ']A' from (select * from (select distinct a.level1module levelmodule, a.level1code   levelcode, null           father_id from DM_EXPORT_PRIVILEGE a where classify = 'tab' union all select distinct a.level2module levelmodule, a.level2code   levelcode, a.level1code   father_id from DM_EXPORT_PRIVILEGE a where classify = 'tab' union all select distinct a.tab_code   levelmodule, a.codes      levelcode, a.level2code father_id from DM_EXPORT_PRIVILEGE a where classify = 'tab')) a, b where 1 = 1 and levelcode like b.type1 || '%' and a.levelcode in (" + s + ") ";
...
window.parent.FS.tabPane.addItem({title:"用户权限配置v2.0",src:"${servletURL}?formlet=cpt_test%2Fauthority%2FHX_EXPORT_AUT_v2.0_0904.frm&p_user="+v_user}) 
]]></Content>
</JavaScript>
</Listener>
...
<TemplateID TemplateID="c86c6623-58b2-460f-996e-28eb22cb9aa5"/>
