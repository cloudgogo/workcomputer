...
<Query>
<![CDATA[select * from (
select a.org_id,b.org_type,b.org_name,b.org_sname,a.time_type_id,a.sign_target sign_target,a.sign_actual sign_actual,a.sign_rate 
from DM_RES_CHECK_SALE_SIGN a inner join dim_org b on a.org_id=b.org_id
where isnew=1 and a.statistics_type='金额'
) res
where res.org_type='集团']]></Query>
</TableData>
...
<Query>
<![CDATA[select org_id,org_type,org_name,org_sname,time_type_id,round(sign_target,1) as sign_target,round(sign_actual,1) as sign_actual,sign_rate,rateall,time_rate from  (
select res.*,
(select sign_rate from (
select a.org_id,b.org_type,b.org_name,b.org_sname,a.time_type_id,a.sign_target sign_target,a.sign_actual sign_actual,a.sign_rate 
from DM_RES_CHECK_SALE_SIGN a inner join dim_org b on a.org_id=b.org_id
where isnew=1 and a.statistics_type='金额'
) res
where res.org_type='集团'
and time_type_id=decode('${periodtype}','当年','年','当季','季','月')
) rateall,(trunc(sysdate)-to_date(to_char(sysdate,'yyyy')||'0101','yyyyMMdd'))/365 time_rate from (
select a.org_id,b.org_type,b.org_name,b.org_sname,a.time_type_id,a.sign_target,a.sign_actual,a.sign_rate 
from DM_RES_CHECK_SALE_SIGN a inner join dim_org b on a.org_id=b.org_id
where isnew=1 and a.statistics_type='金额'
) res
where  
...
order by nvl(sign_rate,0) desc,nvl(sign_actual,0) desc ,nvl(sign_target,0) desc
)
where  rownum<=10
order by /*nvl(sign_rate,0) desc,*/nvl(sign_actual,0) desc ,nvl(sign_target,0) desc
]]></Query>
...
<Query>
<![CDATA[
 select datetype||'-'||CASE WHEN sum(amount)over(partition by datetype)<1 THEN '0' ELSE '' END ||round(sum(amount)over(partition by datetype),1) datetype, org_classify_id||'(亿元)' org_classify_id, WEEK_START WEEK_START,round(amount,1) amount,sum(amount)over(partition by datetype) amountsum
   from (select datetype, org_classify_id, WEEK_START, sum(amount) amount
           from (SELECT  ltrim(substr(W.WEEK_START,5,2),'0')||'.'||ltrim(substr(W.WEEK_START,7,2),'0')||'~'||ltrim(substr(W.WEEK_end,5,2),'0')||'.'||ltrim(substr(W.WEEK_end,7,2),'0') datetype,
                        W.WEEK_START,
                        w.org_classify_id,
                  ...
                  WHERE 1 = 1
                    and ind_type='认购'
                    AND W.DATA_DATE =
                        (SELECT MAX(DATA_DATE) FROM DM_RES_CHECK_TREND_WEEK  where ind_type='回款')) res
          group by datetype, org_classify_id, WEEK_START
         
          ORDER BY res.WEEK_START desc) res
  where rownum <= 24
  order by res.org_classify_id , res.WEEK_START
]]></Query>
</TableData>
<TableData name="5" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="datadate"/>
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=sql("oracle_test","select max(data_date) from DM_RES_CHECK_STORE_SELL",1,1)]]></Attributes>
</O>
</Parameter>
<Parameter>
...
<Query>
<![CDATA[ select * from  (
  select o.org_name, o.org_sname org_sname, round(s.STORE_SALE_RATE,1) STORE_SALE_RATE
   ...
   where s.org_id = o.org_id
     ...
     ${if(ordertype="库存不足","and sTORE_SALE_RATE<2","and sTORE_SALE_RATE>6")}
     order by STORE_SALE_RATE ${if(ordertype="库存积压","desc","asc")}
     )res
     where rownum<=10
     order by STORE_SALE_RATE ${if(ordertype="库存积压","desc","asc")}]]></Query>
</TableData>
<TableData name="6" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
...
<Query>
<![CDATA[   select round(case when sell_house<0 and nvl(new_house,0)=0 then 0 when sell_house<0 and nvl(new_house,0)>0 then new_house else sell_house end,1) sell_house,round(new_house,1) new_house,round(case when cycle<0 then 0 else cycle end,1) cycle,res.* from  (
  select o.org_name, o.org_sname,S.*
    from DM_RES_CHECK_SUPPLY_SELL s, dim_org o
   where s.org_id = o.org_id
     
     ${if(len(area)=0,"and  o.org_sname='华夏幸福'","and  o.org_sname='"+area+"' and org_type='区域'")}
     
     --and o.org_name='产业新城'
     --and substr(data_month,4,1)='M'
     order by nvl(s.cycle,0) desc 
     )res
          order by data_month 
]]></Query>
</TableData>
...
<O>
<![CDATA[periodtype;depttype;ordertype;]]></O>
</ColumnConfig>
<ColumnConfig name="CANSHU" isKey="false" skipUnmodified="false">
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(len($periodtype)=0,"",$periodtype+";")+
if(len($depttype)=0,"",$depttype+";")+
if(len($ordertype)=0,"",$ordertype+";")]]></Attributes>
</O>
</ColumnConfig>
<ColumnConfig name="CANSHUMINGYUCANSHU" isKey="false" skipUnmodified="false">
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(len($periodtype)=0,"","periodtype:"+$periodtype+";")+
if(len($depttype)=0,"","depttype:"+$depttype+";")+
if(len($ordertype)=0,"","ordertype:"+$ordertype+";")]]></Attributes>
</O>
</ColumnConfig>
...
<Condition class="com.fr.data.condition.FormulaCondition">
<Formula>
<![CDATA[if($ordertype=$ordertype99,1<0,0<1)]]></Formula>
</Condition>
</JoinCondition>
<JoinCondition join="1">
<Condition class="com.fr.data.condition.FormulaCondition">
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=$ordertype]]></Attributes>
</O>
</Parameter>
</Parameters>
<Content>
<![CDATA[this.options.form.getWidgetByName("periodtype99").setValue(periodtype);
this.options.form.getWidgetByName("depttype99").setValue(depttype);
this.options.form.getWidgetByName("ordertype99").setValue(ordertype);]]></Content>
</JavaScript>
</JavaScript>
</Listener>
...
<BoundsAttr x="472" y="96" width="80" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.Label">
...
<BoundsAttr x="471" y="69" width="80" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.Label">
...
<BoundsAttr x="473" y="40" width="80" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<text>
<![CDATA[${if(len(area) = 0, "", area + "区域")}]]></text>
</RichChar>
...
<BoundsAttr x="496" y="337" width="157" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report4_t"/>
<WidgetAttr description="">
<PrivilegeControl/>
</WidgetAttr>
<Margin top="0" left="0" bottom="0" right="0"/>
...
<BoundsAttr x="9" y="342" width="180" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report4_l"/>
<WidgetAttr description="">
<PrivilegeControl/>
</WidgetAttr>
<Margin top="0" left="0" bottom="0" right="0"/>
...
<BoundsAttr x="819" y="337" width="27" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[723900,951840,723900,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[762000,2592000,2743200,2857500,2743200,2592000,648000,2592000,432000,936000,864000,720000,2592000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<CellGUIAttr>
<ToolTipText>
<![CDATA[显示库存不足或库存积压前十名的区域]]></ToolTipText>
</CellGUIAttr>
<CellPageAttr/>
<Expand/>
</C>
...
<![CDATA[#0]]></Format>
<AxisLabelCount value="=1"/>
<AxisRange maxValue="=max(value(6, 3)) * 1.4"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
...
<HtmlLabel customText="function(){ return this.category+this.seriesName+this.value+&apos;亿元&apos;;}" useHtml="false" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
<GI>
<AttrBackground>
...
<VanChartColumnPlotAttr seriesOverlapPercent="0.0" categoryIntervalPercent="20.0" fixedWidth="false" columnWidth="15" filledWithImage="false" isBar="false"/>
</VanChartPlot>
...
<Content>
<![CDATA[=TOINTEGER(mid($$$,5,2))+"月"]]></Content>
</CategoryPresent>
</Top>
...
<CategoryName value="DATA_MONTH"/>
<ChartSummaryColumn name="SELL_HOUSE" function="com.fr.data.util.function.NoneFunction" customName="可售房源(亿元)"/>
<ChartSummaryColumn name="NEW_HOUSE" function="com.fr.data.util.function.NoneFunction" customName="新增房源(亿元)"/>
</MoreNameCDDefinition>
</DefinitionMap>
...
<CategoryName value="DATA_MONTH"/>
<ChartSummaryColumn name="CYCLE" function="com.fr.data.util.function.NoneFunction" customName="去化周期(月)"/>
</MoreNameCDDefinition>
</DefinitionMap>
</DefinitionMapList>
</CustomDefinition>
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[="区域存销比预警" + if(ordertype = "库存不足", "(<2个月)", "(>6个月)")]]></Attributes>
</O>
<TextAttr>
<Attr alignText="0">
<FRFont name="微软雅黑" style="0" size="80" foreground="-1"/>
...
<ColorList>
<OColor colvalue="-13324042"/>
<OColor colvalue="-16713985"/>
<OColor colvalue="-74446"/>
<OColor colvalue="-94926"/>
...
<HtmlLabel customText="function(){ return this.value+&apos;月&apos;;}" useHtml="false" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
<GI>
<AttrBackground>
<Background name="ColorBackground" color="-16777216"/>
...
<ChartSummaryColumn name="STORE_SALE_RATE" function="com.fr.data.util.function.NoneFunction" customName="STORE_SALE_RATE"/>
</MoreNameCDDefinition>
</DefinitionMap>
<DefinitionMap key="line">
...
<BoundsAttr x="9" y="337" width="837" height="137"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report3_t"/>
<WidgetAttr description="">
<PrivilegeControl/>
</WidgetAttr>
<Margin top="0" left="0" bottom="0" right="0"/>
...
<BoundsAttr x="9" y="199" width="180" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[0,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[2592000,2592000,2743200,2743200,2743200,2743200,2592000,2592000,2592000,2592000,2592000,2592000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
</C>
<C c="0" r="1" cs="10" rs="9">
<O t="CC">
<LayoutAttr selectedIndex="0"/>
<ChangeAttr enable="false" changeType="button" timeInterval="5" buttonColor="-8421505" carouselColor="-8421505" showArrow="true">
<TextAttr>
...
<AxisLabelCount value="=1"/>
<AxisRange maxValue="=max(value(&quot;4&quot;,5))*1.4"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=1000000" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
<HtmlLabel customText="function(){ return this; }" useHtml="false" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
...
<HtmlLabel customText="function(){ return this.seriesName+this.value+&apos;亿元&apos;;}" useHtml="false" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
<GI>
<AttrBackground>
<Background name="ColorBackground" color="-16777216"/>
...
<DefinitionMapList>
<DefinitionMap key="column">
<OneValueCDDefinition seriesName="ORG_CLASSIFY_ID" valueName="AMOUNT" function="com.fr.data.util.function.NoneFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[4]]></Name>
...
<BoundsAttr x="9" y="195" width="837" height="135"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report1_t"/>
<WidgetAttr description="">
<PrivilegeControl/>
</WidgetAttr>
<Margin top="0" left="0" bottom="0" right="0"/>
...
<BoundsAttr x="9" y="37" width="180" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report1"/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[304800,864000,1295400,952500,952500,0,288000,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[720000,2743200,2160000,1981200,1440000,2743200,2160000,1828800,1440000,720000,2592000,2592000,2592000,2743200,2743200,2743200,2743200,2743200,2743200,2592000,2592000,2592000,864000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<CellGUIAttr>
<ToolTipText>
<![CDATA[按实际签约金额排序，显示前10名]]></ToolTipText>
</CellGUIAttr>
<CellPageAttr/>
<Expand/>
</C>
...
<AxisLabelCount value="=1"/>
<AxisRange maxValue="=max(value(&quot;2&quot;, 6)) * 1.3"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
...
<![CDATA[#0%]]></Format>
<AxisLabelCount value="=1"/>
<AxisRange maxValue="=max(value(&quot;2&quot;, 8)) * 1.3"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴2" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
...
<VanChartColumnPlotAttr seriesOverlapPercent="0.0" categoryIntervalPercent="20.0" fixedWidth="false" columnWidth="20" filledWithImage="false" isBar="false"/>
</VanChartPlot>
<VanChartPlot class="com.fr.plugin.chart.line.VanChartLinePlot">
<VanChartPlotVersion version="20170715"/>
<GI>
...
<CategoryName value="ORG_SNAME"/>
<ChartSummaryColumn name="SIGN_TARGET" function="com.fr.data.util.function.NoneFunction" customName="目标(亿元)"/>
<ChartSummaryColumn name="SIGN_ACTUAL" function="com.fr.data.util.function.NoneFunction" customName="实际(亿元)"/>
</MoreNameCDDefinition>
</DefinitionMap>
<DefinitionMap key="line">
<MoreNameCDDefinition>
...
<CategoryName value="ORG_SNAME"/>
<ChartSummaryColumn name="SIGN_RATE" function="com.fr.data.util.function.NoneFunction" customName="完成率"/>
<ChartSummaryColumn name="RATEALL" function="com.fr.data.util.function.NoneFunction" customName="集团完成率"/>
<ChartSummaryColumn name="TIME_RATE" function="com.fr.data.util.function.NoneFunction" customName="按时需完成率"/>
</MoreNameCDDefinition>
</DefinitionMap>
</DefinitionMapList>
</CustomDefinition>
...
<C c="2" r="3" s="6">
<O t="DSColumn">
<Attributes dsName="1" columnName="SIGN_TARGET"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
<![CDATA[TIME_TYPE_ID]]></CNAME>
<Compare op="0">
<O>
<![CDATA[年]]></O>
</Compare>
...
<C c="2" r="4" s="6">
<O t="DSColumn">
<Attributes dsName="1" columnName="SIGN_ACTUAL"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<GaugeDetailStyle>
<GaugeDetailStyleAttr horizontalLayout="false" needleColor="-1" slotBackgroundColor="-12963316" antiClockWise="true"/>
<MapHotAreaColor>
...
<C c="4" r="6" rs="3" s="12">
<O t="DSColumn">
<Attributes dsName="1" columnName="SIGN_RATE"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<Style imageLayout="1">
<FRFont name="微软雅黑" style="0" size="112" foreground="-16712961"/>
<Background name="NullBackground"/>
<Border/>
</Style>
<Style imageLayout="1">
...
<Style imageLayout="1">
<FRFont name="微软雅黑" style="0" size="64" foreground="-23296"/>
<Background name="NullBackground"/>
<Border/>
</Style>
<Style imageLayout="1">
...
<Format class="com.fr.base.CoreDecimalFormat">
<![CDATA[#0%]]></Format>
<FRFont name="微软雅黑" style="0" size="48" foreground="-16713985"/>
<Background name="NullBackground"/>
<Border/>
</Style>
</StyleList>
...
<BoundsAttr x="9" y="33" width="837" height="155"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[144000,864000,914400,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
...
<BoundsAttr x="9" y="2" width="597" height="25"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[="数据截止日期："+sql("oracle_test","SELECT SUBSTR(max(data_date),1,4)||'-'||SUBSTR(max(data_date),5,2)||'-'||SUBSTR(max(data_date),7,2) data_date from  DM_RES_CHECK_SALE_SIGN",1,1)]]></Attributes>
</O>
<PrivilegeControl/>
<Expand/>
</C>
...
<BoundsAttr x="617" y="2" width="228" height="27"/>
</Widget>
<Sorted sorted="false"/>
<MobileWidgetList>
...
<Widget widgetName="report1_t"/>
<Widget widgetName="periodtype99"/>
<Widget widgetName="depttype99"/>
<Widget widgetName="ordertype99"/>
<Widget widgetName="report3"/>
<Widget widgetName="report3_t"/>
<Widget widgetName="report4"/>
<Widget widgetName="report0"/>
<Widget widgetName="report4_l"/>
<Widget widgetName="report4_t"/>
</MobileWidgetList>
...
<TemplateID TemplateID="9578d11f-0d7d-4876-8683-912c1127aa5b"/>
