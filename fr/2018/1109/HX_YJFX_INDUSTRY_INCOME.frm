...
datetable as
 (
  ...
  select distinct to_char(p.period_date, 'yyyy') || 'Q0' ||
                   ...
                   decode(to_char(p.period_date, 'Q'), '1', '一', '2', '二', '3', '三', '4', '四'  ) || '季度' description,
...
index1 as (
  ...
  from dim_index i
 where i.index_id in ('bcdc0e84a15a4b82831fa8ef7daad520')
  )
...
select code,
  ...
  left join dm_mcl_acct a
   ...
   and a.index_id in ( 'bcdc0e84a15a4b82831fa8ef7daad520' )
...
DIM_DATEKJ AS (
SELECT 
		CASE WHEN '当年'='当年' THEN PERIOD_YEAR
				 WHEN '当季'='当年' THEN PERIOD_QUARTER
				 WHEN '当月'='当年' THEN PERIOD_MONTH END AS CALIBER ,--找到我当前时间参数口径（当年、当季、当月）
		'1' as ORDER_CALIBER
...
SELECT
		...
		CASE WHEN '当年'='当年' THEN PERIOD_YEAR
				 WHEN '当季'='当年' THEN PERIOD_QUARTER
				 WHEN '当月'='当年' THEN PERIOD_MONTH END AS DIM_CALIBER,--口径
		CASE WHEN '当年'='当年' THEN PERIOD_QUARTER
				 WHEN '当季'='当年' THEN PERIOD_MONTH
				 WHEN '当月'='当年' THEN WEEK_NBR_IN_MONTH END AS DIM_CALIBER_S --口径2
...
SELECT 
		...
		CASE WHEN '当年'='当年' THEN substr(date1.date1,1,4) ||'Q0'||substr(b.DIM_CALIBER_S,1,1) 
				 WHEN '当季'='当年' THEN substr(date1.date1,1,4) ||'M'||substr(b.DIM_CALIBER_S,1,2) 
				 WHEN '当月'='当年' THEN substr(date1.date1,1,4) ||'M'||substr(date1.date1,5,2)||'W'||substr(b.DIM_CALIBER_S,2,1) 
...
,ZB AS (select '目标' as zhibiao, '1' as xh
    from dual
  UNION ALL
  select '实际' as zhibiao, '2' as xh
    from dual), --指标维度
    DATE_INDEX AS (
SELECT INDEX_ID,INDEX_NAME,ORDER_KEY FROM DIM_INDEX 
WHERE INDEX_ID='bcdc0e84a15a4b82831fa8ef7daad520' ---产服收入
),--指标维度
DATE_INDEX2 AS (
SELECT INDEX_ID,INDEX_NAME,ORDER_KEY FROM DIM_INDEX 
WHERE father_id='bcdc0e84a15a4b82831fa8ef7daad520' ---无形资产/固定资产
),--指标维度2

DIM_ORF_HX AS(
SELECT ORG_ID,ORG_NAME FROM DIM_ORG 
where org_id = '${org}'--${org}E0A3D386-D5C8-FB22-18DE-4424D49363B1
)--组织维度
，target as (
---产服收入目标
SELECT 
...
FROM DIM_DATES D
LEFT JOIN  DATE_INDEX IND
on 1=1
left join  DIM_ORF_HX hx
on 1=1
left join dm_mcl_acct acc
on acc.PERIOD_TYPE_ID =d.Statistical_time
and ind.index_id=acc.index_id
AND hx.ORG_ID = ACC.ORG_ID
),
acc as (
select * from (
select a.CALIBER, --时间口径
                       ...
                       case
                         when f.zhibiao = '目标' then
                          --decode(d.index_id, '0b551c99b8c045ddac7576e66bd51e98', '目标', '')
                          
                          decode(d.index_id, 'dedc936e00f64bad95af2ca3b1c8adc7', '目标', '')
                         else
                          d.index_name
                       end index_name,                      
                       d.INDEX_ID, --指标id
                       ...
                       case
                         when f.zhibiao = '目标' then
                          ...
                          round(sum(nvl(e.ACTUAL_VALUE, 0)))
                       end value
                
                  from DIM_DATES a --时间维度
                  LEFT JOIN DIM_ORF_HX c --组织维度
                    ON 1 = 1
                  LEFT JOIN DATE_INDEX2 d --指标维度
                    ON 1 = 1
                  LEFT JOIN ZB f
                    ON 1 = 1
                  LEFT JOIN DM_MCL_ACCT e
...
t1.caliber,
       t1.order_caliber,
       t1.STATISTICAL_TIME,
       t1.index_name, t1.index_id,
       t1.order_key,
       t1.zhibiao,
       t1.xh,
       case
         when t1.zhibiao = '目标' then
           decode(t1.index_name, '目标', SUM(nvl(tar.TARGET_VALUE, 0)), 0)
       else
          round(sum(nvl(t1.value, 0)))
       end value

from acc t1
left join target tar
 on t1.caliber = tar.caliber
 and t1.index_name = tar.index_name
 group by 
 t1.caliber,
       t1.order_caliber,
       t1.STATISTICAL_TIME,
       ...
       t1.zhibiao,
       t1.xh
       ORDER BY t1.ORDER_CALIBER, t1.ORDER_KEY desc, t1.xh)
 --select * from acc_data
 ,sum_data as (
  select CALIBER,zhibiao, sum(value)  value1 from acc_data
group by CALIBER,zhibiao
 )
 
 select  
 t.CALIBER||'-' as CALIBER,t.order_caliber,t.STATISTICAL_TIME,t.index_name,t.order_key,
      -- t.zhibiao,
       t.xh,t.value, t.index_id,
       t.zhibiao||'-'||decode(t1.value1,0,'',t1.value1) as zhibiao,
       t1.value1 as sumvalue
 from acc_data  t
 left join sum_data  t1
 on t.CALIBER=t1.CALIBER
and t.zhibiao=t1.zhibiao
ORDER BY t.ORDER_CALIBER,  t.xh, t.ORDER_KEY desc
...
select index_name,  c.index_id,
 ...
 where 1 = 1
   ...
   and c.index_id in ( 'bcdc0e84a15a4b82831fa8ef7daad520' )
   and b.org_id = '${org}'
...
select o.org_id, o.org_name, i.index_id, i.index_name, a.actual_value
 ...
 where a.org_id = o.org_id
   ...
   and a.index_id = 'bcdc0e84a15a4b82831fa8ef7daad520'
   and a.period_type_id = case '当年'
         when '当年' then
          substr(date1.date1, 1, 4)
         when '当季' then
...
<![CDATA[with org_ind as
 (select o.org_id,
         ...
         ind.index_name,
         ind.index_unit
    from dim_org o, dim_index ind
   where 1 = 1
     and o.org_id = '${org}'
     and ind.index_classify = '财务类'
     and ind.father_id='bcdc0e84a15a4b82831fa8ef7daad520' )

select a.*, acc.index_id,
       acc.period_type_id,
       acc.target_value target_value,
       ROUND(nvl(acc.actual_value, 0),0) actual_value,
       
       case
         when nvl(acc.target_value, 0)=0 then 0
           else
       nvl(acc.actual_value, 0) / nvl(acc.target_value, 0) end rate_value
  from org_ind a
  left join dm_mcl_acct acc
    on a.org_id = acc.org_id
   and a.index_id = acc.index_id
      
   --and acc.period_type_id ='2018'

   and acc.period_type_id= case '${periodtype}'
         when '当年' then
          substr('${yearmonth}', 1, 4)
         when '当季' then
          substr('${yearmonth}', 1, 4) || 'Q0' || to_char(sysdate , 'Q')
         when '当月' then
          substr('${yearmonth}', 1, 4) || 'M' || to_char(sysdate , 'MM')
       end
      ]]></Query>
</TableData>
...
DATE_INDEX AS (
SELECT INDEX_ID,INDEX_NAME,ORDER_KEY FROM DIM_INDEX 
WHERE INDEX_ID='bcdc0e84a15a4b82831fa8ef7daad520' 
--UNION
--SELECT INDEX_ID,INDEX_NAME,ORDER_KEY FROM DIM_INDEX 
...
-- 参数yearmonth： REPLACE(DATEDELTA(today(),-1),"-","")
]]></Query>
</TableData>
<TableData name="ZZGD_KJ2" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="org"/>
<O>
...
select period_type_id,
               ...
               index_name,
               index_id,
               ${if(len(valuetype)=0,'actual_value',switch(valuetype,"实际",'actual_value',"完成率", 'round(rate_value/100,2)'))} as zhi,
               '${valuetype}'  as typea
...
select a.period_type_id,
               ...
               index_name,
               c.index_id,
               round(nvl(a.actual_value, 0), 2) actual_value,
               case
                 when nvl(a.target_value, 0) = 0 then
                  0
                 else
                  round(nvl(a.actual_value, 0) / nvl(a.target_value, 0) * 100,
                        1)
               end rate_value
      
          from dm_mcl_acct a, dim_org_jxjl b, dim_index c, date1
         where 1 = 1
           and a.org_id = b.org_id
           and a.index_id = c.index_id
           and c.index_id = 'bcdc0e84a15a4b82831fa8ef7daad520'
           and a.period_type_id = (select substr(max(data_date),1,4)  from dm_mcl_acct)
           ${if(len(depttype) = 0, "and b.org_level ='2'",switch(depttype, '事业部',"and b.org_level ='2'",'区域', "and b.org_level ='3'")) }

         order by ${switch(valuetype,"",'actual_value',"实际",'actual_value',"完成率",'rate_value')}  desc nulls last
         )
 where rownum <= 10 
 order by zhi asc
]]></Query>
...
<BoundsAttr x="204" y="10" width="60" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<O>
<![CDATA[产服收入（管理口径）分析]]></O>
</ColumnConfig>
<ColumnConfig name="CANSHUMING" isKey="false" skipUnmodified="false">
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if($ishx="Y", "org;periodtype;depttype;valuetype", "org;periodtype")]]></Attributes>
</O>
</ColumnConfig>
...
if($ishx="Y", if(len($depttype)=0 ,"",$depttype+";")+if(len($valuetype)=0 ,"",$valuetype), "" )]]></Attributes>
</O>
</ColumnConfig>
...
IF($ishx="Y", if(len($depttype)=0,"","depttype:"+$depttype)+if(len($valuetype)=0,"","valuetype:"+$valuetype), "" )]]></Attributes>
</O>
</ColumnConfig>
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(len($info)=0,"", $info+":"+if($info="产服收入（管理口径）业绩完成详情表", "ThreeLevelPage/HX_YJFX_INCOME_THREE01.cpt", 
if($info="产服收入（管理口径）项目明细表", "ThreeLevelPage/HX_YJFX_INCOME_THREE02.cpt", "HX_YJFX/INDUSTRY/HX_YJFX_INDUSTRY_INCOME_PM.cpt")
))]]></Attributes>
</O>
</ColumnConfig>
<ColumnConfig name="DATA_DATE" isKey="false" skipUnmodified="false">
...
<BoundsAttr x="307" y="3" width="31" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(sql("oracle_test", "select org_level from  dim_org_jxjl where  org_id= '" + $org + "'", 1, 1)=1, "Y", "N")]]></Attributes>
</O>
</widgetValue>
<LabelAttr verticalcenter="true" textalign="0" autoline="true"/>
<FRFont name="SimSun" style="0" size="72"/>
...
<BoundsAttr x="280" y="3" width="30" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<![CDATA[762000,228600,723900,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[144000,2628900,0,288000,2590800,2247900,4381500,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<![CDATA[762000,228600,723900,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[1104900,2628900,0,2590800,2247900,3695700,4038600,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="1" r="0" s="0">
<O>
<![CDATA[当年]]></O>
...
<Background name="ImageBackground" specifiedImageWidth="-1.0" specifiedImageHeight="-1.0" layout="2">
<IM>
<![CDATA[lO<9(kN.ld@UNU%p%320!n%iL/B>R2,V+fARKBHt:D_'5ME,Tn/ei,@i,Q<+bMBgmEu,LJm/
fudG27\4Os(>=[M3OA/q3ir8ftH6<D/X]AfV5,;I$p^nn&<XMZW(XolO:DX0C7g#na<'P8c7F
T);Y4,!X]A/r*G/j@\7"4`Nj7ZYrH/m2b@"?M)78`(h)Q;mU>$rU\fGHIK;YcBb;uPS-3+~
]]></IM>
</Background>
</HighlightAction>
</Highlight>
...
<BoundsAttr x="13" y="28" width="209" height="22"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.Label">
...
<ColumnWidth defaultValue="2743200">
<![CDATA[2743200,1008000,4533900,144000,1008000,3024000,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
var title="产服收入（管理口径）业绩完成详情表";
window.parent.FS.tabPane.addItem({title:title,src:"${servletURL}?reportlet=ThreeLevelPage%2FHX_YJFX_INCOME_THREE01.cpt&op=view&org="+org})
]]></Content>
</JavaScript>
</NameJavaScript>
<NameJavaScript name="当前决策报表对象2">
...
var title="产服收入（管理口径）项目明细表";
window.parent.FS.tabPane.addItem({title:title,src:"${servletURL}?reportlet=ThreeLevelPage%2FHX_YJFX_INCOME_THREE02.cpt&op=view&org="+org})
]]></Content>
</JavaScript>
</NameJavaScript>
<NameJavaScript name="当前决策报表对象2">
...
<Style imageLayout="1">
<FRFont name="SimSun" style="0" size="72" foreground="-16735489"/>
<Background name="NullBackground"/>
<Border/>
</Style>
</StyleList>
...
<BoundsAttr x="549" y="237" width="296" height="22"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<BoundsAttr x="13" y="231" width="205" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="tiaozhuan1_c_c_c"/>
<WidgetAttr description="">
<PrivilegeControl/>
</WidgetAttr>
<Margin top="0" left="0" bottom="0" right="0"/>
...
<![CDATA[720000,723900,723900,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[1008000,2971800,2743200,2743200,2743200,2743200,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0" s="0">
<O t="Image">
<IM>
...
var title="产服收入（管理口径）排名详情表";
window.parent.FS.tabPane.addItem({title:title,src:"${servletURL}?reportlet=HX_YJFX%2FINDUSTRY%2FHX_YJFX_INDUSTRY_INCOME_PM.cpt&op=view&comtype="+depttype+"&valuetype="+valuetype})
...
<Attributes name="info"/>
<O>
<![CDATA[产服收入（管理口径）排名详情表]]></O>
</Parameter>
</Parameters>
<TargetFrame>
<![CDATA[_blank]]></TargetFrame>
<Features/>
<realateName realateValue="tiaozhuan1_c_c_c"/>
<linkType type="1"/>
</JavaScript>
</JavaScript>
</NameJavaScript>
...
<BoundsAttr x="786" y="3" width="65" height="22"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<Listener event="afterinit">
...
<ColumnWidth defaultValue="2743200">
<![CDATA[432000,5760000,2880000,2880000,2880000,2880000,720000,3744000,3744000,3744000,3744000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[864000,216000,1008000,723900,723900,723900,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[8382000,288000,1872000,288000,1872000,2160000,1872000,288000,1872000,288000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0" s="0">
<O>
<![CDATA[产服收入（管理口径）区域分布]]></O>
<PrivilegeControl/>
<HighlightList>
<Highlight class="com.fr.report.cell.cellattr.highlight.DefaultHighlight">
<Name>
...
<NameJavaScript name="CopyOfCopyOfCopyOf当前决策报表对象6">
<JavaScript class="com.fr.form.main.FormHyperlink">
<JavaScript class="com.fr.form.main.FormHyperlink">
<Parameters>
<Parameter>
...
<BoundsAttr x="616" y="1" width="232" height="46"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<BoundsAttr x="13" y="1" width="236" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report3"/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[288000,432000,1584000,432000,2160000,2160000,2160000,2160000,1008000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
...
<HtmlLabel customText="function(){ return this.seriesName+&apos;&lt;br/&gt;&apos;+FR.contentFormat(this.value,&quot;#.0&quot;)+&apos;亿 &apos;+FR.contentFormat(this.percentage,&quot;#%&quot;);}" useHtml="true" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
</labelDetail>
</AttrLabel>
</Attr>
...
<AxisRange maxValue="=max(value(&quot;bar&quot;, 7)) * 1.3"/>
...
<ChartDefinition>
<OneValueCDDefinition seriesName="TYPEA" valueName="ZHI" function="com.fr.data.util.function.NoneFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[bar]]></Name>
...
<HtmlLabel customText="function(){ return this.seriesName+&apos;&lt;br/&gt;&apos;+FR.contentFormat(this.value,&quot;#&quot;)+&apos;个 &apos;+FR.contentFormat(this.percentage,&quot;#%&quot;);}" useHtml="true" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
</labelDetail>
</AttrLabel>
</Attr>
...
<BoundsAttr x="611" y="1" width="240" height="220"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<BoundsAttr x="292" y="1" width="144" height="27"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[723900,1728000,1728000,1728000,1728000,1728000,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[2160000,2160000,2160000,2160000,2160000,2743200,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0" cs="5">
<PrivilegeControl/>
<Expand/>
...
<Attr4VanChart floating="true" x="78.0" y="0.0" limitSize="false" maxHeight="15.0" isHighlight="true"/>
</Legend4VanChart>
<DataSheet>
<GI>
<AttrBackground>
...
<![CDATA[#0%]]></Format>
<AxisLabelCount value="=1"/>
<AxisRange minValue="=0" maxValue="=if( or(max(ZZGD_KJ2.group(rate_value))=0, len(max(ZZGD_KJ2.group(rate_value)))=0), 1, max(ZZGD_KJ2.group(rate_value))*1.5 )"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[723900,2160000,2160000,2160000,2160000,2160000,723900,723900,723900,723900,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[2160000,2160000,2160000,2160000,2160000,2743200,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0" cs="5">
...
<BoundsAttr x="287" y="9" width="325" height="174"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report2"/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[288000,432000,2160000,2160000,2160000,2160000,2160000,2160000,1008000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[2592000,2592000,2592000,2592000,2592000,2743200,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<AxisLabelCount value="=1"/>
<AxisRange maxValue="=if(OR(len(ZZGD_KJ.group(sumvalue)) = 0, ZZGD_KJ.group(sumvalue) = 0), 0, max(ZZGD_KJ.group(sumvalue)) * 1.3)"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
...
<HtmlLabel customText="   function(){ return FR.contentFormat(this.category.substring(this.category.indexOf(&apos;-&apos;)+1,this.category.length),&quot;#0&quot;);}" useHtml="true" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
</labelDetail>
</AttrLabel>
</Attr>
...
<BoundsAttr x="287" y="1" width="315" height="220"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<WidgetName name="report1"/>
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[288000,432000,2160000,2160000,2160000,2160000,2160000,2160000,2160000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[288000,576000,2016000,2016000,1728000,2880000,2880000,432000,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<C c="5" r="10" s="11">
<O t="DSColumn">
<Attributes dsName="ds3" columnName="ACTUAL_VALUE"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<C c="4" r="11" s="10">
<O t="DSColumn">
<Attributes dsName="ds3" columnName="RATE_VALUE"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<C c="5" r="11">
<O t="DSColumn">
<Attributes dsName="ds3" columnName="TARGET_VALUE"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<Style imageLayout="1">
<FRFont name="微软雅黑" style="0" size="216" foreground="-16711681"/>
<Background name="NullBackground"/>
<Border/>
</Style>
<Style imageLayout="1">
...
<![CDATA[288000,432000,2160000,2160000,2160000,2160000,2160000,2160000,1008000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[288000,1152000,2016000,2016000,1728000,2160000,2160000,432000,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<BoundsAttr x="8" y="1" width="270" height="220"/>
</Widget>
<Sorted sorted="false"/>
<MobileWidgetList>
...
<TemplateID TemplateID="adc4161c-8a0e-440c-b88b-ea1dce132f81"/>
