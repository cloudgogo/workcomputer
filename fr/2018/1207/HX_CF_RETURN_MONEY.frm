...
<TableDataMap>
<TableData name="nav" class="com.fr.data.impl.DBTableData">
<Parameters/>
...
SELECT '' as  FJID , '2' AS  ZIID, '回款' as NAMES FROM dual
UNION  all
select * from JXJL_DH where  fjid = 2
...
<TableData name="yusdb" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="nav"/>
<O>
...
<![CDATA[--净现金预实对比
with data_time as
 (select substr(a.data_date, 1, 4) year1,
         substr(a.data_date, 1, 4) || to_char(to_date(a.data_date, 'YYYYMMDD'), 'Q') quar,
         substr(a.data_date, 1, 4) || to_char(to_date(a.data_date, 'YYYYMMDD'), 'MM') mon
    from dm_ncf_netcashflow a
   where 1 = 1
     and a.data_date is not null
     and rownum = 1)


select  *  from (
select '目标' type1, a.gname , nvl(round(a.target,1),0) as zhi,  case when a.target<>0 then  a.fact / a.target  else 0 end wcl 
  from dm_ncf_netcashflow a, dim_ncf_index i, data_time t
 where 1 = 1
      
   and a.index_id = i.guid
   and a.guid = '${org}'
  ${SWITCH(nav, "回款", " and i.serialnumber='JXJL07' ", 
        "经营性回款", " and i.serialnumber='JXJLJY01' ",
        "投资活动现金流入", " and i.serialnumber='JXJLTZ01' ",
        "筹资活动现金流入", " and i.serialnumber='JXJLCZ01' ") }
...
---参数projecct
   ...
   ${SWITCH(periodtype,  "当年",  " and a.date_type='年' and a.date_string=t.year1 ",
                "当季",  " and a.date_type='季' and a.date_string=t.quar ",
                "当月",  " and a.date_type='月' and a.date_string=t.mon ") }

union
     
select '实际' type1, a.gname  , nvl(round(a.fact,1),0) as zhi,  case when a.target<>0 then  a.fact / a.target  else 0 end wcl 
  from dm_ncf_netcashflow a, dim_ncf_index i, data_time t
 where 1 = 1
...
<TableData name="wancqs" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="nav"/>
<O>
...
<Attributes name="org"/>
<O>
<![CDATA[A80C0CB8-E296-491B-B222-C2D89BC74AEC]]></O>
</Parameter>
<Parameter>
<Attributes name="periodtype"/>
<O>
...
data_time as
 (select *
    from (
          select distinct '当年' period_type,
		...
		union all
          select distinct '当月' period_type,
                          p.week_nbr_in_month period_name,
                          substr(base.period_year, 1, 4) || 
                          substr(base.period_month, 1, 2) ||
                          substr(p.week_nbr_in_month, 2, 1) period_type_id,
                          '2' rn
            from dim_period p, base
           where 1 = 1
             and p.period_year = base.period_year
             and p.period_quarter = base.period_quarter
             and p.period_month = base.period_month)
             
   where 1 = 1
    and period_type='${periodtype}'
    --${switch(periodtype,"当季","and period_type='当季'","当月","and period_type='当季'","当年","and period_type='当年'")}
    
    ),


data1 as
 (select i.names, i.serialnumber, a.date_string, a.target, a.fact,i.ordernum
    from dm_ncf_netcashflow a, dim_ncf_index i
   where 1 = 1
      and a.index_id = i.guid
     and a.guid = '${org}'
${SWITCH(nav, "回款", " and i.serialnumber in ('JXJLCZ01','JXJLTZ01','JXJLJY0101')", 
   "经营性回款", " and i.serialnumber in ('JXJLJY010101','JXJLJY010102','JXJLJY010103') ",
   "投资活动现金流入", " and i.serialnumber in ('JXJLTZ0101','JXJLTZ0102','JXJLTZ0103','JXJLTZ0104','JXJLTZ0105') ",
   "筹资活动现金流入", " and i.serialnumber in ('JXJLCZ0101','JXJLCZ0102','JXJLCZ0103') ") }
     --and a.calibre='1b97deb312be4c2d836db933a0edc9c2'
...
---参数projecct
         ...
         (select org_id
                  from dim_org o
                 where (org_classify_id in
                       ('产业新城（国内）', '产业新城（国际）') or org_level = 1)
                   and o.org_id = a.guid)
...
select *
  from (select '目标' type1, period_type, period_name,
               d.names, d.serialnumber, d.date_string,
               nvl(round(d.target, 1), 0) as zhi,
               case
                 when d.fact <> 0 then
                  d.target / d.fact
                 else
                  0
               end as wcl,d.ordernum
          from data_time t
          left join data1 d
            on t.period_type_id = d.date_string
         
        union all
        
        select '实际' type1, period_type, period_name,
               d.names, d.serialnumber, d.date_string,
               nvl(round(d.fact, 1), 0) as zhi,
               case
                 when d.fact <> 0 then
                  d.target / d.fact
                 else
...
order by date_string asc,to_number(ordernum)  asc ,period_name


]]></Query>
</TableData>
<TableData name="wancjg" class="com.fr.data.impl.DBTableData">
<Parameters>
<Parameter>
<Attributes name="nav"/>
<O>
...
with base as
 (select p.period_key,
         ...
         p.week_nbr_in_month
    from dim_period p
   where p.period_key = /*'20180919' */(select a.data_date
                                 from dm_ncf_netcashflow a
                                where 1 = 1
                                  and a.data_date is not null
                                  and rownum = 1)
...
data_time as
 (select *
    from (select '当年' period_type,
                 base.period_year period_name,
                 substr(base.period_year, 1, 4) period_type_id,
                 '1' rn
            from base
          
          union all
          select '当季' period_type,
                 base.period_quarter period_name,
                 substr(base.period_year, 1, 4) ||
                 substr(base.period_quarter, 1, 1) period_type_id,
                 '1' rn
            from base
            
            union all
          select '当月' period_type,
                 base.period_month period_name,
                 substr(base.period_year, 1, 4) ||
                 substr(base.period_month, 1, 2) period_type_id,                    
                 '1' rn
            from base
...
data1 as
 (select i.names, i.serialnumber, a.date_string, a.target, nvl(a.fact,0) as fact
    from dm_ncf_netcashflow a, dim_ncf_index i
   where 1 = 1
      and a.index_id = i.guid
     and a.GUID = '${org}'
...
select  period_type, period_name, d.names,
d.serialnumber, d.date_string,nvl(round(d.target,1),0) as target , nvl(round(d.fact,1),0) as fact          
	  from data_time t
	  left join data1 d
	    on t.period_type_id = d.date_string
    where  d.target <> 0 or d.fact <> 0]]></Query>
</TableData>
...
with data_time_m as
 (select substr(a.data_date, 1, 4) year1,
         substr(a.data_date, 1, 4) ||
         to_char(to_date(a.data_date, 'YYYYMMDD'), 'Q') quar,
         substr(a.data_date, 1, 4) ||
         to_char(to_date(a.data_date, 'YYYYMMDD'), 'MM') mon
    from dm_ncf_netcashflow a
   where 1 = 1
     and a.data_date is not null
     and rownum = 1),
data_time_q as
 (select year1,
         quar,
         case
           when substr(quar, 5) = 1 then
            year1 || '01'
           when substr(quar, 5) = 2 then
            year1 || '04'
           when substr(quar, 5) = 3 then
            year1 || '07'
           when substr(quar, 5) = 4 then
            year1 || '10'
         end mon
    from data_time_m),
org as
...
select serialnumber, names, guid, org_name as gname, time_type, target, fact, wcl,  ordernum,levels,org_num
  from (
        --当年
        select distinct a.guid, i.names, I.serialnumber,i.levels, i.ordernum,
                         a.gname,
                         a.date_string,
                         case
                           when length(a.date_string) = 4 then
                            a.date_string || '年'
                           when length(a.date_string) = 5 then
                            substr(a.date_string, 5, 1) || '季度'
                           when length(a.date_string) = 6 then
                            to_number(substr(a.date_string, 5, 6)) || '月'
                         end time_type,
                         a.target,
                         a.fact,
                         a.complete_rate wcl
          from dm_ncf_netcashflow a, data_time_m, dim_ncf_index i
         where 1 = 1
           and a.guid in (select org_id from org)
           and a.index_id=i.guid
           --and i.levels in (2,3,4)
             ${SWITCH(nav, "回款", " and i.serialnumber in ('JXJL07' ,'JXJLJY01', 'JXJLJY0101', 'JXJLJY010101', 'JXJLJY010102', 'JXJLJY010103', 'JXJLJY0102', 'JXJLTZ01', 'JXJLCZ01') ",   
"经营性回款", " and ( i.levels in (2,4,5) and (i.serialnumber = 'JXJLJY0101' or i.serialnumber like 'JXJLJY01%') )  ",
"投资活动现金流入", "and (i.serialnumber = 'JXJLTZ01' or i.serialnumber like 'JXJLTZ01%') ",
"筹资活动现金流入", " and (i.levels in (2,3) and (i.serialnumber like 'JXJLCZ01%' or i.serialnumber = 'JXJLCZ01')) ") }
...
---参数projecct
         ...
         (select org_id
           ...
           and a.date_string like data_time_m.year1 || '%'
           and length(a.date_string) in (4, 5)
           and '${periodtype}' = '当年'
...
---参数projecct
         ...
         (select org_id
           ...
           and a.date_string in
               (data_time_m.year1, data_time_m.quar, data_time_m.mon)
           and '${periodtype}' = '当月'
        ...
        select distinct a.guid, i.names, I.serialnumber,i.levels, i.ordernum,
                         ...
                         a.fact,
                         a.complete_rate wcl
          from dm_ncf_netcashflow a, data_time_q, dim_ncf_index i
         where 1 = 1
           and a.guid in (select org_id from org)
           and a.index_id=i.guid           
          -- and i.levels in (2,3,4)
...
---参数projecct
         ...
         (select org_id
           ...
           and (a.date_string in (data_time_q.year1, data_time_q.quar) or
               a.date_string in
               (data_time_q.mon, data_time_q.mon + 1, data_time_q.mon + 2))
 ...
 where 1 = 1
   and guid = org_id
 order by to_number(date_string), to_number(org_num), to_number(ordernum)
]]></Query>
</TableData>
<TableData name="nav_Tree" class="com.fr.data.impl.RecursionTableData">
<markFields>
<![CDATA[1]]></markFields>
<parentmarkFields>
<![CDATA[0]]></parentmarkFields>
<markFieldsName>
<![CDATA[ZIID]]></markFieldsName>
<parentmarkFieldsName>
<![CDATA[FJID]]></parentmarkFieldsName>
<originalTableDataName>
<![CDATA[nav]]></originalTableDataName>
</TableData>
...
select serialnumber, names, guid, gname, time_type, target, fact, wcl,  ordernum,levels,org_num
  from (
        --当年
        select distinct a.guid, i.names, I.serialnumber,i.levels, i.ordernum,
                         a.gname,
         ...
         where 1 = 1
           ...
           --and i.levels in (2,3,4)
            ${SWITCH(nav, "回款", " and i.serialnumber in ('JXJL07' ,'JXJLJY01', 'JXJLJY0101', 'JXJLJY010101', 'JXJLJY010102', 'JXJLJY010103', 'JXJLJY0102', 'JXJLTZ01', 'JXJLCZ01') ",   
"经营性回款", " and ( i.levels in (4,5) and (i.serialnumber = 'JXJLJY01' or i.serialnumber like 'JXJLJY01%') )  ",
"投资活动现金流入", "and (i.serialnumber = 'JXJLTZ01' or i.serialnumber like 'JXJLTZ01%') ",
"筹资活动现金流入", " and (i.levels in (2,3) and (i.serialnumber like 'JXJLCZ01%' or i.serialnumber = 'JXJLCZ01')) ") }
...
select  *  from (
select '目标' type1, a.gname , nvl(round(a.target,1),0) as zhi,  case when a.fact<>0 then a.target / a.fact else 0 end wcl 
  from dm_ncf_netcashflow a, dim_ncf_index i, data_time t
 where 1 = 1
   ...
   and a.guid = '${org}'
  ${SWITCH(nav, "回款", " and i.serialnumber='JXJL07' ", 
        "经营性回款", " and i.serialnumber='JXJLJY0101' ",
        "投资活动现金流入", " and i.serialnumber='JXJLTZ01' ",
        "筹资活动现金流入", " and i.serialnumber='JXJLCZ01' ") }
...
select '实际' type1, a.gname  , nvl(round(a.fact,1),0) as zhi,  case when a.fact<>0 then a.target / a.fact else 0 end wcl 
  from dm_ncf_netcashflow a, dim_ncf_index i, data_time t
 where 1 = 1
...
<Attributes>
<![CDATA[=sql("oracle_test","select  distinct DATA_DATE from  dm_mcl_acct",1,1)]]></Attributes>
</O>
</Parameter>
<Parameter>
<Attributes name="periodtype"/>
...
<Content>
<![CDATA[setTimeout(
 function() {$('.fr-trigger-btn-up').css('background', '#2cc5d8')},0);//.fr-trigger-btn-up ]]></Content>
</JavaScript>
</Listener>
<Listener event="afterinit">
<JavaScript class="com.fr.js.JavaScriptImpl">
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=sql("oracle_test", " select to_char(to_date(max(data_date), 'YYYYMMDD'), 'YYYY-MM-DD') from dm_ncf_netcashflow ", 1)]]></Attributes>
</O>
</Parameter>
</Parameters>
<Content>
...
<BoundsAttr x="473" y="10" width="125" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<BoundsAttr x="404" y="10" width="70" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<BoundsAttr x="628" y="10" width="60" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<BoundsAttr x="249" y="10" width="125" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.Label">
...
<BoundsAttr x="210" y="10" width="40" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TreeComboBoxEditor">
...
<Parameters/>
<Content>
<![CDATA[//var nav=this.options.form.getWidgetByName("nav").getValue(); 
//alert(nav);
//if(nav=="净现金流"){//alert(nav);
//}
//else if(nav=="回款"){
//		window.parent.parent.FS.tabPane.addItem({title:"回款",src:"${servletURL}?formlet=doc%2Ft%2FHX_CF_RETURN_MONEY.frm&nav="+FR.cjkEncode(nav)})
//}
////else if(nav==211){
////	window.parent.parent.FS.tabPane.addItem({title:"销售回款",src:"${servletURL}?reportlet="+FR.cjkEncode("doc/xu_bak/净现金流/xsxhk_new20180920.cpt&op=view")})
////}
//else if(nav=="经营性回款"){
//		window.parent.parent.FS.tabPane.addItem({title:"经营性回款",src:"${servletURL}?formlet=doc%2Ft%2FHX_CF_RETURN_MONEY.frm&nav="+FR.cjkEncode(nav)})
//}
//else if(nav=="投资活动现金流入"){
//		window.parent.parent.FS.tabPane.addItem({title:"投资活动现金流入",src:"${servletURL}?formlet=doc%2Ft%2FHX_CF_RETURN_MONEY.frm&nav="+FR.cjkEncode(nav)})
//}
//else if(nav=="筹资活动现金流入"){
//		window.parent.parent.FS.tabPane.addItem({title:"筹资活动现金流入",src:"${servletURL}?formlet=doc%2Ft%2FHX_CF_RETURN_MONEY.frm&nav="+FR.cjkEncode(nav)})
//}
//else if(nav=="支出"){
//		window.parent.parent.FS.tabPane.addItem({title:"支出",src:"${servletURL}?formlet=doc%2Ft%2FHX_CF_RETURN_MONEY.frm&nav="+FR.cjkEncode(nav)})
//}else{
//	_g().parameterCommit();
//}
]]></Content>
...
<Content>
<![CDATA[//var nav=this.options.form.getWidgetByName("nav").getValue(); 
//window.parent.parent.FS.tabPane.addItem({title:nav);]]></Content>
</JavaScript>
</Listener>
<WidgetName name="nav"/>
<LabelName name="导航："/>
...
<isPerformanceFirst performanceFirst="false"/>
<Dictionary class="com.fr.data.impl.TableDataDictionary">
<FormulaDictAttr kiName="NAMES" viName="NAMES"/>
<TableDataDictAttr>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[nav_Tree]]></Name>
...
<TagModified tag="org" modified="true"/>
<TagModified tag="org_c" modified="true"/>
</NameTagModified>
<WidgetNameTagMap>
<NameTag name="nav" tag="导航："/>
<NameTag name="Search" tag="当月"/>
...
<O>
<![CDATA[nav;org;project;periodtype;valuetype]]></O>
</ColumnConfig>
<ColumnConfig name="CANSHU" isKey="false" skipUnmodified="false">
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(len($nav)=0,"",$nav+";")+
if(len($org)=0,"",$org_name+";")+
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[=if(len($nav)=0,"","nav："+$nav+";")+
if(len($org)=0,"","org："+$org_name+";")+
...
<![CDATA[=if(len($info)=0,"",
$info+":"+SWITCH($info, 
"回款详情", "/ThreeLevelPage/jxjl_hk.cpt", 
"经营性回款详情", "/ThreeLevelPage/jxjl_jyxhk.cpt",
"投资活动现金流入详情", "/ThreeLevelPage/jxjl_tzhdxjlr.cpt",
"筹资活动现金流入详情 ", "/ThreeLevelPage/jxjl_czxhdxjlr.cpt"
)
)]]></Attributes>
</O>
</ColumnConfig>
...
<Formula>
<![CDATA[if( $nav = $nav99  , 1<0 , 0<1 )]]></Formula>
</Condition>
</JoinCondition>
...
<Content>
<![CDATA[this.options.form.getWidgetByName("nav99").setValue(nav);
this.options.form.getWidgetByName("org99").setValue(org);
...
<BoundsAttr x="101" y="2" width="39" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="411" y="6" width="33" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="314" y="6" width="33" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="377" y="5" width="33" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="501" y="3" width="36" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="462" y="3" width="21" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.TextEditor">
...
<BoundsAttr x="168" y="2" width="38" height="21"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
...
<Content>
<![CDATA[if(nav=="回款"){
window.parent.FS.tabPane.addItem({title:nav+"明细",src:"${servletURL}?reportlet=ThreeLevelPage/jxjl_hk.cpt&op=view&org="+org1})
}

else if(nav=="经营性回款"){
window.parent.FS.tabPane.addItem({title:nav+"明细",src:"${servletURL}?reportlet=ThreeLevelPage/jxjl_jyxhk.cpt&op=view&org="+org1})
}
else if(nav=="投资活动现金流入"){
window.parent.FS.tabPane.addItem({title:nav+"明细",src:"${servletURL}?reportlet=ThreeLevelPage/jxjl_tzhdxjlr.cpt&op=view&org="+org1})
}
else if(nav=="筹资活动现金流入"){
window.parent.FS.tabPane.addItem({title:nav+"明细",src:"${servletURL}?reportlet=ThreeLevelPage/jxjl_czxhdxjlr.cpt&op=view&org="+org1})
}]]></Content>
</JavaScript>
</NameJavaScript>
</NameJavaScriptGroup>
...
<BoundsAttr x="789" y="215" width="59" height="24"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
<Listener event="afterinit">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,1008000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[432000,5715000,8153400,2743200,2880000,2880000,2880000,2880000,2880000,2880000,2880000,2880000,2880000,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<C c="7" r="1" cs="3" s="1">
<O t="DSColumn">
<Attributes dsName="mingxb" columnName="TIME_TYPE"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
<![CDATA[TIME_TYPE]]></CNAME>
<Compare op="10">
<O>
<![CDATA[季度]]></O>
</Compare>
...
<C c="3" r="3" s="2">
<O t="DSColumn">
<Attributes dsName="mingxb" columnName="LEVELS"/>
<Complex/>
<RG class="com.fr.report.cell.cellattr.core.group.FunctionGrouper"/>
<Parameters/>
</O>
...
<C c="6" r="3" s="6">
<O t="DSColumn">
<Attributes dsName="mingxb" columnName="WCL"/>
<Condition class="com.fr.data.condition.CommonCondition">
<CNUMBER>
<![CDATA[0]]></CNUMBER>
<CNAME>
...
<Highlight class="com.fr.report.cell.cellattr.highlight.DefaultHighlight">
<Name>
<![CDATA[CopyOfCopyOfCopyOf条件属性4]]></Name>
<Condition class="com.fr.data.condition.ListCondition">
<JoinCondition join="0">
<Condition class="com.fr.data.condition.FormulaCondition">
<Formula>
...
<BoundsAttr x="8" y="213" width="846" height="246"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[216000,1008000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[288000,4320000,4320000,4320000,2160000,2160000,288000,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<OColor colvalue="-8562985"/>
<OColor colvalue="-16750485"/>
<OColor colvalue="-7431500"/>
...
<AxisLabelCount value="=1"/>
<AxisRange minValue="=if(min(value(&quot;wancqs&quot;, 7)) &gt; 0, 0, min(value(&quot;wancqs&quot;, 7)) * 1.3)" maxValue="=max(value(&quot;wancqs&quot;, 7)) * 1.4"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
...
<ChartDefinition>
<OneValueCDDefinition seriesName="NAMES" valueName="ZHI" function="com.fr.data.util.function.SumFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[wancqs]]></Name>
</TableData>
<CategoryName value="PERIOD_NAME"/>
...
<ChartDefinition>
<OneValueCDDefinition seriesName="NAMES" valueName="WCL" function="com.fr.data.util.function.NoneFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[wancqs]]></Name>
...
<BoundsAttr x="546" y="0" width="306" height="200"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[216000,1008000,1296000,1296000,1296000,1296000,1296000,1296000,1296000,1296000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[288000,3600000,3600000,3600000,3600000,432000,1143000,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<HtmlLabel customText="function(){   var d=this.value/10000;  var a=&quot;&lt;span style=&apos;font-size:12rem;font-family:微软雅黑;&apos;&gt;&quot;+this.seriesName+&quot;&lt;/span&gt;&lt;br/&gt;&lt;span style=&apos;font-size:12rem;font-family:微软雅黑;&apos;&gt;&quot;+FR.contentFormat(d, &apos;#0.00&apos;)+&quot;亿元&lt;/span&gt;&quot;;  return a; }" useHtml="true" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
</AttrToolTipContent>
</labelDetail>
</AttrLabel>
</Attr>
...
<ChartDefinition>
<OneValueCDDefinition seriesName="NAMES" valueName="FACT" function="com.fr.data.util.function.SumFunction">
<Top topCate="-1" topValue="-1" isDiscardOtherCate="false" isDiscardOtherSeries="false" isDiscardNullCate="false" isDiscardNullSeries="false"/>
<TableData class="com.fr.data.impl.NameTableData">
<Name>
<![CDATA[wancjg]]></Name>
...
<BoundsAttr x="234" y="0" width="302" height="200"/>
</Widget>
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.container.WTitleLayout">
...
<RowPrivilegeControl/>
<RowHeight defaultValue="723900">
<![CDATA[216000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,1440000,723900]]></RowHeight>
<ColumnWidth defaultValue="2743200">
<![CDATA[288000,2160000,2160000,3600000,3600000,288000,1440000,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<O t="Formula" class="Formula">
<Attributes>
<![CDATA[="<span style='font-size:12px;color:#ffffff;font-family:微软雅黑;'>完成率&nbsp;&nbsp;&nbsp;&nbsp;</span><span style='font-size:18px;color:#00ffff;font-family:微软雅黑;'>" + ROUND(C12[C12:!1]A* 100,0) + "%</span>"]]></Attributes>
</O>
<PrivilegeControl/>
<CellGUIAttr showAsHTML="true"/>
<CellPageAttr/>
...
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="X轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
<HtmlLabel customText="function(){ return this; }" useHtml="true" isCustomWidth="false" isCustomHeight="false" width="50" height="50"/>
<alertList/>
<customBackgroundList/>
</VanChartAxis>
</XAxisList>
...
<TextAttr>
<Attr alignText="0">
<FRFont name="Verdana" style="0" size="64" foreground="-3809797"/>
</Attr>
</TextAttr>
<AxisLabelCount value="=1"/>
<AxisRange minValue="=if(len(value(&quot;yusdb&quot;, 3)) = 0, 0, if(min(value(&quot;yusdb&quot;, 3) &gt; 0), 0, min(value(&quot;yusdb&quot;, 3)) * 0.63))" maxValue="=if(len(value(&quot;yusdb&quot;, 3)) = 0, 0, if(max(value(&quot;yusdb&quot;, 3)))&lt;0,1,max(value(&quot;yusdb&quot;, 3)) * 1.3+1)"/>
<AxisUnit201106 isCustomMainUnit="false" isCustomSecUnit="false" mainUnit="=0" secUnit="=0"/>
<ZoomAxisAttr isZoom="false"/>
<axisReversed axisReversed="false"/>
<VanChartAxisAttr mainTickLine="0" secTickLine="0" axisName="Y轴" titleUseHtml="false" autoLabelGap="true" limitSize="false" maxHeight="15.0" commonValueFormat="true" isRotation="false"/>
...
<ChartSummaryColumn name="ZHI" function="com.fr.data.util.function.NoneFunction" customName="值"/>
</MoreNameCDDefinition>
</ChartDefinition>
</Chart>
<tools hidden="true" sort="false" export="false" fullScreen="false"/>
...
<C c="2" r="11">
<O t="DSColumn">
<Attributes dsName="yusdb" columnName="WCL"/>
<Complex/>
<RG class="com.fr.report.cell.cellattr.core.group.FunctionGrouper"/>
<Parameters/>
</O>
...
<BoundsAttr x="8" y="0" width="216" height="200"/>
</Widget>
<Sorted sorted="false"/>
<MobileWidgetList>
...
<TemplateID TemplateID="ebc72be8-17b1-431a-bdf4-93ee0538ab65"/>
