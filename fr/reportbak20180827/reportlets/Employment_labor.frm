...
(
 ...
 /*UNION ALL
	 ...
	 SELECT 	
	 TREE_NODE_NUM,
	 CASE WHEN TDATE='2018. 03' THEN '当前' ELSE TDATE END AS TDATE,
	 TREE_NODE,
	 PARENT_NODE,
	 DESCR,
	 TREE_NODES,
	 ...
	 ORDER BY TREE_NODE_NUM,TDATE]]></Query>
</TableData>
...
(
 ...
 ROUND(CASE WHEN LAG(ZRS,1,0) over ( partition by TREE_NODE order by TDATE)=0 
	             THEN  NULL
				 ELSE ZRS/LAG(ZRS,1,0) over ( partition by TREE_NODE order by TDATE) 
					END -1,4) AS RATIO	
	 FROM TT3
	 WHERE TREE_NODE<>'HX_HEAD'
	 AND (TDATE='201712' OR TDATE='201803')
	 ORDER BY TREE_NODE_NUM,TDATE
	 )
	 SELECT * FROM TT5
	 WHERE TDATE='2018. 03']]></Query>
</TableData>
</TableDataMap>
...
<![CDATA[720000,3771900,2160000,2160000,2160000,1728000,2743200,2743200,2743200,2743200,2743200]]></ColumnWidth>
<CellElementList>
<C c="0" r="0">
<PrivilegeControl/>
<Expand/>
...
<TemplateID TemplateID="42ff23f2-ddb2-470e-b0b2-fc7b329cd007"/>
</Form>
